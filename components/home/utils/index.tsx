...
export const updateProfileLocationStateZip = async (uid: string, userStateZip: StateZipProps) => {
    ...
    async function updateProfileLocationStateZip() {
        return await fireDb.collection(Profile).doc(uid).set({
            locationStateZip: userStateZip
        }, { merge: true })
            .then(() => { return userStateZip })
            .catch((err) => {
                console.log(err)
            })
    }
    ...
    const dbStateZip: StateZipProps | undefined = await fireDb.collection(Profile).doc(uid).get()
        .then((doc: any) => {
            const { locationStateZip } = doc.data();

            if (locationStateZip) {
                return locationStateZip
            } else {
                return;
            }
        })
    ...
    if (dbStateZip) {
        ...
        if (userStateZip.state !== dbStateZip.state || userStateZip.zip !== dbStateZip.zip) {
            //remove the path of previous location in the Location collection
            await fireDb.collection(Locations).doc(dbStateZip.state).collection(dbStateZip.zip).doc(uid).delete()
                .catch((err) => {
                    console.log(err)
                })
